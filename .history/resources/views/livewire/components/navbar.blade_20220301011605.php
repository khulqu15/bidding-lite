        ...
        <div class="row w-100">
            <div class="col-6 px-5">
                <div class="d-inline-block cursor-point" onclick="window.location = '{{ route("home") }}'">
                    ...
                    <h4 class="viga d-inline-block position-relative ml-3 d-md-inline-block d-none" style="top: 5px; left: 10px">Tawarin</h4>
                </div>
            </div>
            <div class="col-6 px-md-0 px-5 text-right">
                @if (Auth::check())
                ...
                @endif
                <div class="burger d-md-none d-inline-block cursor-point" onclick="toggleNavbar()">
                    <div></div>
                    <div></div>
                </div>
                <div class="d-md-inline-block d-none">
                    <div class="d-flex @if(Auth::check()) mx-5 @endif">
                        <div role="button" onclick="window.location = '{{ route('home') }}'" class="menu mx-2 position-relative" style="top: 4px">Home</div>
                        <div role="button" onclick="window.location = '{{ route('products') }}'" class="menu mx-2 position-relative" style="top: 4px">Product</div>
                        @if (!Auth::check())
                            <div role="button" onclick="window.location = '{{ route('contact') }}'" class="menu mx-2 position-relative" style="top: 4px">Contact</div>
                        @else
                            <div role="button" onclick="window.location = '{{ route('contact') }}'" class="menu ml-2 mr-5 position-relative" style="top: 4px">Contact</div>
                        @endif
                        @if (!Auth::check())
                        <div class="menu mx-1">
                            <button onclick="window.location = '{{ route('login') }}'" class="btnm-primary py-2 px-3 btn-sm viga px-5">Login</button>
                        </div>
                        <div class="menu mx-1">
                            <button onclick="window.location = '{{ route('register') }}'" class="btnm-light py-2 px-3 btn-sm viga px-5 mx-2">Register</button>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="col-md-6 maxm-height {{ Auth::check() ? 'px-md-5 py-md-2' : 'p-md-5' }} p-4 text-right">
                    ...
                    <div class="px-5 position-relative">
                        ...
                        <div class="position-relative" style="right: -10px">
                            <button onclick="window.location = '{{ route('login') }}'" class="btnm-primary viga px-5">Login</button>
                            <button onclick="window.location = '{{ route('register') }}'" class="btnm-light viga px-5 mx-2">Register</button>
                        </div>
                        @endif
                    </div>
                </div>

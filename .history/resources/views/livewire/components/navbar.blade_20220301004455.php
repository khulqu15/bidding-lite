            ...
            <div class="col-6 px-md-0 px-5 text-right">
                ...
                <div class="d-md-inline-block d-none">
                    <div class="d-flex @if(Auth::check()) mx-5 @endif">
                        <div role="button" onclick="window.location = '{{ route('home') }}'" class="menu mx-2 position-relative" style="top: 4px">Beranda</div>
                        <div role="button" onclick="window.location = '{{ route('products') }}'" class="menu mx-2 position-relative" style="top: 4px">Produk</div>
                        @if (!Auth::check())
                            <div role="button" onclick="window.location = '{{ route('contact') }}'" class="menu mx-2 position-relative" style="top: 4px">Kontak</div>
                        @else
                            <div role="button" onclick="window.location = '{{ route('contact') }}'" class="menu ml-2 mr-5 position-relative" style="top: 4px">Kontak</div>
                        @endif
                        @if (!Auth::check())
                        <div class="menu mx-1">
                            <button onclick="window.location = '{{ route('login') }}'" class="btnm-primary py-2 px-3 btn-sm viga px-5">Masuk</button>
                        </div>
                        <div class="menu mx-1">
                            <button onclick="window.location = '{{ route('register') }}'" class="btnm-light py-2 px-3 btn-sm viga px-5 mx-2">Daftar</button>
                        </div>
                        @endif
                    </div>
                </div>
